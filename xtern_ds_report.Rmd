...
The $\textit{app\_content\_rating}$ column contains integers with a "+" character appended to the end. I removed the pluses and converted the resulting strings to integers. This will allow me to take averages and analyze this vector if I need to.
```{r preprocessing3}
...
## Data Analysis
Now that the data has been processed, I can begin to make sense of it. I began by making density plots of every column in the dataset. The $\textit{DataExplorer}$ library makes it easy to view all plots at once.
...
The 'Games' category stands out as an outlier. The number of 'Games' apps (3832) is over 7 times greater than the number of 'Entertainment' apps (534). I created a separate dataset that contains everything but apps labeled 'Games' in case the outlier affects future observations.
```{r analysis4}
