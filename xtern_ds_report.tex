...
\NormalTok{appStore.noGames <-}\StringTok{ }\KeywordTok{na.omit}\NormalTok{(appStore.noGames)                       }\CommentTok{# remove games (now NA)}
\end{Highlighting}
\end{Shaded}
...
\KeywordTok{plot}\NormalTok{(appStore.prices, }\DataTypeTok{xlab=}\StringTok{"Price"}\NormalTok{)                         }\CommentTok{# plot distribution of prices}
\end{Highlighting}
\end{Shaded}
...
I removed games from the data and plotted it to see what kind of
difference it made (if any). Nothing changed drastically because the
price data is still right-skewed.

\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{plot}\NormalTok{(}\KeywordTok{as.data.frame}\NormalTok{(}\KeywordTok{table}\NormalTok{(appStore.noGames}\OperatorTok{$}\NormalTok{app_price)), }\DataTypeTok{xlab=}\StringTok{"Price"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
